...
namespace FSM7_eventdriven.StudentOwnedStates {
#region RestAndSleep
    public class RestAndSleep : State<Student> {
        ...
        public bool OnMessage(Student student, Telegram telegram) {return false;}
    }
#endregion
...
#region 
    public class GlobalMessageReceive : State<Student> {
        private static readonly GlobalMessageReceive _instance = new GlobalMessageReceive();
        public static GlobalMessageReceive Instance => _instance;
        ...
        public bool OnMessage (Student student, Telegram telegram) {
            student.PrintText($"Receive Message: sender({telegram.sender}), receiver({telegram.receiver})");
            if(telegram.message.Equals("GO_PCROOM")) {
                if(student.CurrentState == PlayAGame.Instance) {
                    student.PrintText($"Send Message: {telegram.receiver}님이 , {telegram.receiver}님에게 ALREADY_PCROOM 전송");
                    ...
                    if(stateIndex == 0) {
                        student.PrintText($"Send Message {telegram.receiver}님이 {telegram.sender}님에게 GO_PCROOM 전송");
                        MessageDispatcher.Instance.DispatchMessage(0, telegram.receiver, telegram.sender, "GO_PCROOM");
                    }
                    else {
                        student.PrintText($"Send Message {telegram.receiver}님이 {telegram.sender}님에게 SORRY 전송");
